...
export default function CheckoutPage() {
  ...
  const totalBeforeGiftCard = discountedSubtotalKobo + finalShippingFeeKobo;
    ...
    try {
      const res = await fetch("/api/gift-cards/validate", {
        ...
        body: JSON.stringify({
          ...
          total_kobo: totalBeforeGiftCard,
        }),
      });
      const data = await res.json();
      if (data.valid) {
        setAppliedGiftCard({
          ...
          deductionKobo: data.deduction_kobo,
        });
        setGiftCardMessage({
          type: "success",
          text: `Gift Card applied! Balance: ${formatKoboToNaira(data.balance_kobo)}`,
        });
      } else {
        setGiftCardMessage({
          type: "error",
      ...
      setGiftCardMessage({
        type: "error",
        text: "Could not validate gift card.",
      });
    } finally {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 h-fit space-y-5">
          ...
          <div className="border-t border-slate-200 pt-4">
            ...
            {appliedGiftCard ? (
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-indigo-50 border border-indigo-200 text-xs">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-indigo-600 shrink-0" />
                  <div>
                    <span className="font-bold text-indigo-950 font-mono">{appliedGiftCard.code}</span>
                    <span className="block text-[11px] text-indigo-700">
                      Balance: {formatKoboToNaira(appliedGiftCard.balanceKobo)} (Deducting {formatKoboToNaira(giftCardDeductionKobo)})
                    </span>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleApplyGiftCard} className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 text-xs border border-slate-300 rounded-lg uppercase font-mono focus:outline-none focus:ring-2 focus:ring-indigo-600"
                  />
                  <button
                    type="submit"
                    disabled={giftCardLoading || !giftCardInput.trim()}
                    className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-semibold rounded-lg transition-colors"
                  >
                    {giftCardLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : "Apply"}
                  </button>
                </div>
                {giftCardMessage && (
                  <p
                    className={`text-[11px] font-medium ${
                      giftCardMessage.type === "success" ? "text-indigo-700" : "text-red-600"
                    }`}
                  >
